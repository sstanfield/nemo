  ...
  Widget build(BuildContext context) {
    List<Widget> children = [
      ...
      new IntEdit(
          ...
          validator: (int v) =>
              (v < 0 || v > 100) ? "Enter gradiant factor 0-100" : null,
          label: "gfLo"),
      new IntEdit(
      ...
      new IntEdit(
          ...
          validator: (int v) => (v < 500 || v > 3000)
              ? "Enter surface pressure in mbar (500-3000)"
              : null,
          label: "ATM Pressure"),
      new IntEdit(
          initialValue: _dive.descentRate,
          onSaved: (int v) => _dive.descentRate = v,
          validator: (int v) => (v < 1 || v > (_dive.metric ? 300 : 900))
              ? "Enter Descent Rate ${_dive.metric ? "M" : "ft"}/min (1-${_dive.metric ? "300" : "900"})"
              : null,
      ...
      new IntEdit(
          ...
          validator: (int v) => (v < 1 || v > (_dive.metric ? 300 : 900))
              ? "Enter Ascent Rate ${_dive.metric ? "M" : "ft"}/min (1-${_dive.metric ? "300" : "900"})"
              : null,
          label: "Ascent (${_dive.metric ? "M" : "ft"}/min):"),
      new IntEdit(
          initialValue: _dive.surfaceInterval,
          onSaved: (int v) => _dive.surfaceInterval = v,
          validator: (int v) => (v < 0 || v > 14400)
              ? "Enter Surface Interval Minutes (0-14,400 (10 days))"
              : null,
          label: "Surface Interval Min"),
    ];
    if (_dive.isCCR()) {
      children.add(new DoubleEdit(
          initialValue: _dive.decoSetpoint,
